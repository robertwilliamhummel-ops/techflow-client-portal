...
/**
 * InvoiceList — admin invoice table.
 *
 * Mobile addition:
 *   A sub-tab bar [ All Invoices | Recurring ] is rendered at the top.
 *   It is hidden on desktop via CSS (.il-subtabs) so the existing desktop
 *   "Recurring" nav tab remains the only entry point there.
 *   On mobile, tapping "Recurring" swaps the invoice table for
 *   RecurringInvoiceList — no new routes, no AdminDashboard changes needed.
 */
function InvoiceList({ user, onCreateNew }) {
  const navigate = useNavigate();

  // ── Mobile sub-tab: 'all' | 'recurring' ──────────────────────────────────
  ...
  return (
    <div className="invoice-list-admin">
      ...
      {/*
       * Mobile-only sub-tab bar — hidden on desktop via CSS.
       * Lets admins access Recurring without cluttering the main mobile nav.
       * On desktop this is always hidden and the desktop "Recurring" nav tab
       * handles that view instead.
       */}
      <div className="il-subtabs">
        <button
          className={`il-subtab ${invoiceSubTab === 'all' ? 'il-subtab--active' : ''}`}
          onClick={() => setInvoiceSubTab('all')}
        >
          <i className="fas fa-file-invoice-dollar"></i> All Invoices
        </button>
        <button
          className={`il-subtab ${invoiceSubTab === 'recurring' ? 'il-subtab--active' : ''}`}
      ...
      {/* ── Recurring sub-view (mobile only) ── */}
